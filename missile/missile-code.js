...
function missileAdvance(the_missile, the_player) {
    ...
    const m_x_dir = the_missile.m_x_dir;
    if (m_x_dir < 0) {
        the_missile.m_x = leftOnBoard(the_missile.m_x, MISSILE_LR_SPEED);
    } else if (m_x_dir > 0) {
        the_missile.m_x = rightOnBoard(the_missile.m_x, MISSILE_LR_SPEED);
    } else {
        the_missile.m_y = backwardOnBoard(the_missile.m_y, MISSILE_FOR_SPEED);
    }
    return the_missile;
}
...
function missileDraw(the_sprite, the_player) {
    ...
    const [the_z_index, missile_relative, x_center_offset, _head_on_view] = objectPlacement(the_sprite, the_player);
