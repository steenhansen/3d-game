...
function drawBullet(degree_90) { // 0..90     1/0 ,, 4/10
  ...
  // 
  // console.log("flat", degree_90, flatten, 0 - 60 + degree_90 * 1.45);  // -60 +89 = 29 
  up_down = 0 - 10 + degree_90 * 1.45;
  //  console.log("flat", degree_90, up_down);  // -60 +89 = 29 

  //  for (i = 0; i < 180; i = i + 0.2) {
  ctx.strokeStyle = black_color;
  ctx.lineWidth = 3;
  center_y = (degree_90 + 10) * 2.5;
  for (i = 0; i < 180; i = i + 1) {
    ...
    // console.log(bot_half + up_down, top_half + up_down);
    ctx.beginPath();




    ctx.moveTo(x1, bot_half + up_down);
    ctx.lineTo(x1, top_half + up_down);
    ctx.stroke();
    ctx.closePath();
    ...
    // ctx.lineWidth = 1;
    // for (j = y1; j > y2; j--) {
    //   let the_color = randBlackGrey(); //////////////  70 
    ...
    //   )
    //   //      ctx.moveTo(x1, j - 70 - up_down);
    //   //     ctx.moveTo(x1, j - up_down);
    //   ctx.moveTo(x1, j);
    //   ctx.lineTo(x1 + x_offset, j + y_offset - 60); // -60 IS THE TOP +70 is the bottom
    //   console.log(x1, j, up_down);
    //   //   ctx.lineTo(x1 + x_offset, j + y_offset - 60); // -60 IS THE TOP +70 is the bottom
    //   ctx.stroke();
    //   ctx.closePath();
    // }
