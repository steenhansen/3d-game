...
function drawBullet(degree_90) { // 0..90     1/0 ,, 4/10
  ...
  up_down = 0 - 60 + degree_90 * 1.45;
  //console.log("flat", degree_90, flatten, up_down);  // -60 +89 = 29 

  //  for (i = 0; i < 180; i = i + 0.2) {
  for (i = 0; i < 180; i = i + 10) {
    x1 = Math.round(center_x + radius * Math.cos(2 * Math.PI * i / 360)) + degree_90 * 256;
    y1 = Math.round(center_y + radius * Math.sin(2 * Math.PI * i / 360) * flatten); // + up_down
    console.log(x1, y1);
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x1 + 10, y1 + 10); // -60 IS THE TOP +70 is the bottom
    ctx.stroke();
    ctx.closePath();
    //   diff = y1 - 128;
    //   y2 = 128 - diff;
