...
function holeOnMiddle(x_center_offset, hole_player_ys) {
  const dist_abs_y = distanceAbsY(hole_player_ys);
  let [left_front_bot, right_front_bot, _back_right_bot, back_left_bot] =
    panels3Middle(x_center_offset, dist_abs_y, HOLE_PIXEL_DEPTH);
  return [left_front_bot, right_front_bot, _back_right_bot, back_left_bot];
}

function createHoleHtml(a_hole, the_player) {
  let [the_z_index, hole_relative, x_center_offset, head_on_view] =
    objectPlacement(a_hole, the_player);
  const hole_player_ys = [a_hole.m_y, the_player.m_y];
  let left_front_bot, right_front_bot, back_RIGHT_bot, back__LEFT__bot;
  if (head_on_view) {
    [left_front_bot, right_front_bot, back_RIGHT_bot, back__LEFT__bot] =
      holeOnMiddle(x_center_offset, hole_player_ys);
  } else if (hole_relative === LEFT_OF_PLAYER) {
    [left_front_bot, right_front_bot, back_RIGHT_bot, back__LEFT__bot] =
      panels3BackRight(x_center_offset, hole_player_ys, HOLE_PIXEL_DEPTH);
  } else {
    [left_front_bot, right_front_bot, back__LEFT__bot, back_RIGHT_bot] =
      panels3BackLeft(x_center_offset, hole_player_ys, HOLE_PIXEL_DEPTH);
  }
