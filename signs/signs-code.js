...
function signDraw(a_sign, the_player) {
  const [the_z_index, sign_relative, x_center_offset, head_on_view] =
  ...
  if (sign_relative === LEFT_OF_PLAYER) {
    ...
    left_mid_right_vlines = signOnRight(
      x_center_offset,
      sign_player_ys,
      head_on_view,
    );
...
function signPolygon(a_polygon, vertical_text, text_color) {
  const [left_front_top, right_front_top, left_front_bot, _right_front_bot] =
    a_polygon;
