...
function columnSvg(a_column, the_player) {
  ...
  if (column_relative !== COLUMN_HIDDEN) {
    difference_y = the_player.y - a_column.y;
...
function frontColumn(x_offset, difference_y) {
  front_right_bot_vanish_point_YYYY = [x_offset + HALF_COLUMN_WIDTH, BOTTOM_CHECKERBOARD];
  front_y_offset = difference_y - HALF_COLUMN_DEPTH;
  right_front_bot = distancedPoint(front_y_offset, front_right_bot_vanish_point_YYYY);
  front_left_bot_vanish_point_XXXX = [x_offset - HALF_COLUMN_WIDTH, BOTTOM_CHECKERBOARD];
...
function columnLeft2(a_column, x_offsets, difference_y) {
  ...
  let the_column = columnInFront([left_vline, middle_vline, right_vline], gradient_left, gradient_front, [difference_x, difference_y]);
  return the_column;
}
...
function columnRight2(a_column, x_offsets, difference_y) {
  ...
  gradient_front = a_column.column_colors.column_front;
  let the_column = columnInFront([left_vline, middle_vline, right_vline], gradient_right, gradient_front, [difference_x, difference_y]);
  return the_column;
}
...
function columnFrontOnly(left_vline, middle_vline, column_front, difference_x, difference_y) {
  ...
  let front_column = svgPolygon([left_front_top, right_front_top, left_front_bot, right_front_bot], column_front);
...
function columnInFront(column_vlines, gradient_side, gradient_front, the_x_y_diffs) {
  ...
  let front_column = svgPolygon([left_front_top, right_front_top, left_front_bot, right_front_bot], gradient_front);
  let side_column = svgPolygon([right_front_top, back_right_top, right_front_bot, back_right_bot], gradient_side);
