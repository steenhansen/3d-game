...
function twirlSides(a_pylon) {
  let { m_grad_side_twirl: side_twirl, m_grad_frnt_twirl: front_twirl } =
    a_pylon;
  side_twirl += 0.25;
...
function pylonDraw(a_pylon, the_player) {
  ...
  const [the_z_index, pylon_relative, x_center_offset, head_on_view] =
    objectPlacement(a_pylon, the_player);
  const pylon_player_ys = [a_pylon.m_y, the_player.m_y];
  let left_mid_right_vlines, side_pylon;
  if (pylon_relative === LEFT_OF_PLAYER) {
    [left_mid_right_vlines, side_pylon] = pylonOnLeft(
      a_pylon,
      x_center_offset,
      pylon_player_ys,
      head_on_view,
    ...
    [left_mid_right_vlines, side_pylon] = pylonOnRight(
      a_pylon,
      x_center_offset,
      pylon_player_ys,
      head_on_view,
  ...
  const front_pylon = pylonFront(
    left_mid_right_vlines,
    pylon_id,
    do_flash,
    dist_abs_y,
    poly_fill,
...
function pylonSidePanel(a_pylon, pylon_vlines, dist_abs_y) {
  ...
  let side_pylon_svg = pylonPolygon(
    right_top_bot,
    side_s_pylon_name,
    do_flash,
    dist_abs_y,
...
function pylonOnRight(a_pylon, x_center_offset, pylon_player_ys, head_on_view) {
  ...
  if (head_on_view) {
    ...
    left_mid_right_vlines = objectRightSide(
      x_center_offset,
      pylon_player_ys,
      PYLON_PIXEL_DEPTH,
    );
...
function getPylonFill(a_pylon, front_or_side) {
  if (g_p_graphics_style !== P_SIMPLE) {
    ...
    const twirl_gradient = twirledGradient(
      a_pylon.m_grad_side_twirl,
      from_color,
      to_color,
    );
